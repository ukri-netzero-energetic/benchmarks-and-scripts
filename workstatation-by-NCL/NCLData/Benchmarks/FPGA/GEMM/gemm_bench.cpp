...
int main(int argc, char** argv) {
    ...
    for (int kernelIndex = 0; kernelIndex < l_numKernel; kernelIndex++) {
        ...
        posix_memalign((void**)&tmp_a, 16384, m * k * sizeof(BLAS_dataType));
        memset(tmp_a, 0, m * k);
        posix_memalign((void**)&tmp_b, 16384, k * n * sizeof(BLAS_dataType));
        memset(tmp_b, 0, k * n);
        posix_memalign((void**)&tmp_c, 16384, m * n * sizeof(BLAS_dataType));
        memset(tmp_c, 0, m * n);
