...
if [ "$1" == "-h" ]; then
  ...
  while [  $n -le 4096 ]; do
    ...
    echo "#############  $n ################"
    sudo ./test.sh
    nice ./build_dir.hw.xilinx_u50_gen3x16_xdma_5_202210_1/gemm_bench.exe $1 $2 $n $n $n /mnt/2UB102/Vitis_Libraries/blas/L3/benchmarks/gemm/memKernel/data/$dataType/ $numKernels | tee log-$n.txt
    sudo ./test.sh
    logs="$logs log-$n.txt"
    n=`expr $n \* 2`
